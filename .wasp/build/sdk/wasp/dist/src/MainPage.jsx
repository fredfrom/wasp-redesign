...
function CodeBlock() {
    ...
    return (<div className="gradient-border noise-overlay rounded-2xl bg-white dark:bg-surface-1 shadow-xl dark:shadow-2xl shadow-amber-900/5 dark:shadow-black/40 overflow-hidden">
      ...
      <div className="p-5 md:p-6 overflow-x-auto">
        {tab === 'wasp' ? (<pre className="font-mono text-[13px] leading-[1.75]" dangerouslySetInnerHTML={{ __html: `<span class="tk-kw">app</span> <span class="tk-type">myApp</span> <span class="tk-br">{</span>
...
<span class="tk-br">}</span>` }}/>) : (<pre className="font-mono text-[13px] leading-[1.75]" dangerouslySetInnerHTML={{ __html: `<span class="tk-kw">model</span> <span class="tk-type">User</span> <span class="tk-br">{</span>
  <span class="tk-prop">id</span>       <span class="tk-type">Int</span>     <span class="tk-fn">@id</span> <span class="tk-fn">@default</span>(<span class="tk-fn">autoincrement</span>())
...
function CodeComparison() {
    ...
    return (<section className={`py-28 md:py-36 border-t ${t.sec}`}>
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-8">
          {comparisons.map((c, i) => (<button key={c.label} onClick={() => { setActiveComp(i); setActiveTab(0); }} className={`px-3 py-1.5 rounded-xl text-[12px] font-medium cursor-pointer transition-all ${activeComp === i
                ? 'bg-wasp-100 dark:bg-wasp-400/10 text-wasp-700 dark:text-wasp-400 border border-wasp-200 dark:border-wasp-400/20'
                : `bg-zinc-100 dark:bg-white/[0.04] ${t.m} border border-transparent hover:border-zinc-200 dark:hover:border-white/[0.08]`}`}>{c.label}</button>))}
        </div>
          ...
          <div className="gradient-border noise-overlay rounded-2xl bg-white dark:bg-surface-1 shadow-xl dark:shadow-2xl shadow-amber-900/5 dark:shadow-black/40 overflow-hidden">
            <div className="flex items-center justify-between border-b border-zinc-100 dark:border-white/[0.05] px-5 py-3">
              <div className="flex items-center gap-1.5">
                {comp.tabs.map((tb, i) => (<button key={tb.name} onClick={() => setActiveTab(i)} className={`px-3 py-1.5 text-[12px] font-medium rounded-lg cursor-pointer transition-all ${i === activeTab
                ? (i === 0 ? 'code-tab-active' : 'bg-zinc-100 dark:bg-white/[0.06] text-zinc-700 dark:text-zinc-300')
                : 'text-zinc-400 dark:text-zinc-600 hover:text-zinc-600 dark:hover:text-zinc-400'}`}>{tb.name}</button>))}
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-[11px] font-mono ${isWasp ? 'text-emerald-600 dark:text-emerald-400' : t.f}`}>{tab.lines} lines of code</span>
                {!isWasp && tab.lines > comp.tabs[0].lines && (<span className="text-[10px] font-medium text-red-500/70 dark:text-red-400/50 bg-red-50 dark:bg-red-500/10 px-2 py-0.5 rounded-md">
...
function CLIDemoBrowserContent({ type, waspVersion }) {
    if (type === 'install')
        return (<div className="p-5 flex flex-col items-center justify-center h-full min-h-[280px]">
    ...
    if (type === 'new')
        return (<div className="p-5 min-h-[280px]">
      ...
      <div className="space-y-1.5">
        {[
            ...
            ].map(f => (<div key={f.name} className={`flex items-center gap-3 px-3 py-2 rounded-lg bg-zinc-50 dark:bg-white/[0.02] border border-zinc-100 dark:border-white/[0.04]`}>
    ...
    if (type === 'clean')
      ...
      <div className="flex flex-wrap justify-center gap-2 text-[9px]">
        {['.wasp/out/', '.wasp/build/', 'node_modules/', '.wasp/cache/'].map(p => (<span key={p} className={`px-2 py-1 rounded-md bg-zinc-50 dark:bg-white/[0.03] ${t.f} border border-zinc-100 dark:border-white/[0.05] line-through`}>{p}</span>))}
      </div>
    </div>);
    return null;
}
    ...
    return (<section className={`py-28 md:py-36 border-t ${t.sec}`}>
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="flex flex-wrap justify-center gap-1.5 mb-8">
          {cliDemoSteps.map((s, i) => (<button key={i} onClick={() => setActiveStep(i)} className={`px-2.5 py-1.5 rounded-lg text-[11px] font-mono cursor-pointer transition-all ${activeStep === i
                ? 'bg-wasp-100 dark:bg-wasp-400/10 text-wasp-700 dark:text-wasp-400 border border-wasp-200 dark:border-wasp-400/20'
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="gradient-border noise-overlay rounded-2xl bg-white dark:bg-surface-1 shadow-xl dark:shadow-2xl shadow-amber-900/5 dark:shadow-black/40 overflow-hidden">
              ...
              <div className="p-4 font-mono text-[11px] leading-[1.7] min-h-[280px] overflow-hidden">
                ...
                {showOutput && (<pre className={`${t.m} mt-2 whitespace-pre-wrap cli-demo-fade-in`}>{step.terminalOutput.replace(/%VERSION%/g, waspVersion)}</pre>)}
              </div>
            </div>
...
function InstallCommand() {
    ...
    return (<section className={`py-20 border-t ${t.sec}`}>
...
function ArchitectureDiagram() {
    ...
    return (<section className={`py-28 md:py-36 border-t ${t.sec}`}>
      <div className="max-w-[1100px] mx-auto px-6">
        ...
        ].map(tech => (<span key={tech.name} className="flex items-center gap-1.5 text-[8.5px] font-mono px-2 py-1 rounded bg-zinc-50 dark:bg-white/[0.02] border border-zinc-200/50 dark:border-white/[0.05] text-zinc-500 dark:text-zinc-500">
...
function FeatureCard({ title, desc, color, icon }) {
    return (<div className="card p-5 group">
...
function FAQ({ q, a, defaultOpen = false }) {
    return (<details className={`group rounded-2xl border ${t.brd} bg-white dark:bg-white/[0.02]`} open={defaultOpen || undefined}>
...
export const MainPage = () => {
    ...
    return (<div className="min-h-screen light-texture">
      ...
      <nav className={`fixed top-0 left-0 right-0 z-50 nav-blur bg-[#fafaf8]/80 dark:bg-surface-0/80 border-b ${t.sec}`}>
        <div className="max-w-[1200px] mx-auto px-6 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8">
            {[['Docs', 'https://wasp.sh/docs'], ['Blog', 'https://wasp.sh/blog'], ['FAQ', '#faq']].map(([label, href]) => (<a key={label} href={href} className={`text-[13px] ${t.link}`}>{label}</a>))}
          </div>
        ...
        {mobileMenuOpen && (<div className={`md:hidden bg-white dark:bg-surface-1 border-b ${t.sec} px-6 py-5`}>
            ...
            <div>
              ...
              <div className="anim-5 flex items-center gap-5">
                ...
                <div className="flex items-center gap-1">
                  {['React', 'Node.js', 'Prisma', 'Vite'].map((tech, i) => (<span key={tech} className="flex items-center">
      ...
      <section className={`py-28 md:py-36 border-t ${t.sec}`}>
        <div className="max-w-[1200px] mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {testimonials.map(tm => (<a key={tm.handle} href={tm.href} target="_blank" rel="noreferrer" className="card p-6 block">
      ...
      <section className={`py-28 md:py-36 border-t ${t.sec}`}>
        ...
        ].map(item => (<div key={item.title} className="card p-6">
      ...
      <section className={`py-28 md:py-36 border-t ${t.sec}`}>
        ...
        ].map(({ stat, label, sub }) => (<div key={label} className={`text-center p-4 rounded-xl bg-zinc-50 dark:bg-white/[0.03] border ${t.brd}`}>
...
<span class="tk-type">Claude:</span> <span class="tk-cmt">Updates main.wasp auth config</span>
      ...
      <section className={`py-28 md:py-36 border-t ${t.sec}`}>
        <div className="max-w-[1200px] mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            {blogPosts.map(post => (<a key={post.title} href={post.href} target="_blank" rel="noreferrer" className="card overflow-hidden block group">
